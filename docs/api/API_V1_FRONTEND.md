...
- 현재 지원 범위는 `오늘`, `내일`, `모레`, `YYYY-MM-DD`, `YYYY년 M월 D일`, `M월 D일`, `M/D`, `이번 주/다음 주 + 요일`, 단독 요일(`금요일 병원`), `오전/오후 N시`, `N시`, `매주 요일` 기반 규칙 파싱이다.
...
- `이번 주 금요일 병원`, `다음주 월요일 오전 10시 치과`처럼 주 단위 상대 요일은 기준 날짜가 속한 주 또는 다음 주의 해당 요일로 생성한다.
