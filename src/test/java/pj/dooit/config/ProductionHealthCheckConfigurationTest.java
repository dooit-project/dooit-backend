...
class ProductionHealthCheckConfigurationTest {
    ...
    void composeBindsOnlyApplicationToLoopback() throws Exception {
        ...
        assertThat(compose).contains("DOOIT_JWT_SECRET");
        assertThat(compose).contains("DOOIT_GUEST_JWT_ACCESS_TOKEN_TTL: ${DOOIT_GUEST_JWT_ACCESS_TOKEN_TTL:-P31D}");
        assertThat(compose).contains("DOOIT_PASSWORD_RESET_TOKEN_TTL: ${DOOIT_PASSWORD_RESET_TOKEN_TTL:-PT30M}");
        assertThat(compose).contains("DOOIT_PASSWORD_RESET_LINK_TEMPLATE:");
        assertThat(compose).contains("APP_BATCH_SCHEDULER_ENABLED: ${APP_BATCH_SCHEDULER_ENABLED:-false}");
        assertThat(Files.readString(Path.of(".env.example"))).contains("DOOIT_GUEST_JWT_ACCESS_TOKEN_TTL=P31D");
    }
    ...
    void composeAndDockerfileConfigureImageTagAndLogRotation() throws Exception {
        ...
        assertThat(compose).contains("image: dooit-backend:${DOOIT_APP_IMAGE_TAG:-local}");
        assertThat(compose).contains("APP_VERSION: ${DOOIT_APP_IMAGE_TAG:-local}");
        assertThat(compose).contains("DOOIT_APP_COMMIT_SHA: ${DOOIT_APP_COMMIT_SHA:-local}");
        assertThat(compose).contains("DOOIT_APP_IMAGE_TAG: ${DOOIT_APP_IMAGE_TAG:-local}");
    ...
    void actuatorReadinessIncludesDatabaseAndSchema() throws Exception {
        ...
        assertThat(Files.readString(Path.of("src/main/java/pj/dooit/config/SchemaHealthIndicator.java")))
    ...
    void productionEnvironmentCheckScriptValidatesConfigurationWithoutPrintingSecrets() throws Exception {
        ...
        assertThat(script).contains("DOOIT_JWT_SECRET must be at least 32 bytes");
    ...
    void databaseBackupAndRestoreScriptsHaveSafetyChecks() throws Exception {
        ...
        assertThat(backup).contains("DOOIT_BACKUP_RETENTION_DAYS:-14");
        assertThat(restore).contains("set -euo pipefail");
        assertThat(restore).contains("DOOIT_CONFIRM_RESTORE");
    ...
    void productionEnvironmentScriptGeneratesJwtSecretWithoutOverwritingExistingValues() throws Exception {
        ...
        assertThat(script).contains("set_env_value \"DOOIT_JWT_ISSUER\"");
        assertThat(script).contains("set_env_value \"DOOIT_JWT_SECRET\"");
        assertThat(script).contains("already has a non-placeholder value; refusing to overwrite .env");
        assertThat(script).contains("set_env_value \"DOOIT_PUBLIC_API_URL\"");
        assertThat(script).contains("validate_https_url");
        assertThat(script).doesNotContain("printf 'DOOIT_JWT_SECRET=%s\\n' \"$jwt_secret\"");
    }
    ...
    void releaseAndRollbackScriptsSupportTaggedImages() throws Exception {
        ...
        assertThat(release).contains("DOOIT_APP_COMMIT_SHA=\"$commit_sha\"");
        assertThat(release).contains("DOOIT_APP_IMAGE_TAG=\"$image_tag\" docker compose build app");
        ...
        assertThat(rollback).contains("DOOIT_APP_COMMIT_SHA=\"$image_tag\"");
        assertThat(rollback).contains("DOOIT_APP_IMAGE_TAG=\"$image_tag\" docker compose up -d --no-build app");
    ...
    void backupLaunchdInstallScriptCreatesDailyBackupAgent() throws Exception {
        ...
        assertThat(script).contains("pj.dooit.backend.backup");
        assertThat(script).contains("StartCalendarInterval");
        assertThat(script).contains("DOOIT_BACKUP_HOUR:-3");
        assertThat(script).contains("DOOIT_BACKUP_MINUTE:-15");
        ...
        assertThat(runbook).contains("launchctl print gui/$(id -u)/pj.dooit.backend.backup");
    }
}
