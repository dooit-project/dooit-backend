...
class OpenApiDocumentationIntegrationTest {
    ...
    void documentationUi_v1ApiDocsAndTagOrder() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.tags[0].name").value("v1 Daily Plan"))
                .andExpect(jsonPath("$.tags[1].name").value("v1 Workspace Invitation"))
                .andExpect(jsonPath("$.tags[2].name").value("v1 Calendar Feed"))
                .andExpect(jsonPath("$.tags[3].name").value("v1 Workspace Task"))
                .andExpect(jsonPath("$.tags[4].name").value("v1 Task Checklist"))
                .andExpect(jsonPath("$.tags[5].name").value("v1 Task Template"))
                .andExpect(jsonPath("$.tags[6].name").value("v1 System"))
                .andExpect(jsonPath("$.tags[7].name").value("v1 Workspace"))
                .andExpect(jsonPath("$.tags[8].name").value("v1 Auth"))
                .andExpect(jsonPath("$.paths['/api/v1/daily-plans/{date}'].get.summary").value("일일 계획 조회"))
                .andExpect(jsonPath("$.paths['/api/v1/daily-plans/{date}'].put.summary").value("일일 계획 저장"))
                .andExpect(jsonPath("$.paths['/api/v1/tasks/{taskId}/checklist-items'].post.summary").value("Task checklist item 생성"))
                .andExpect(jsonPath("$.tags[*].name").value(hasItem("v1 Workspace")))
                .andExpect(jsonPath("$.tags[*].name").value(hasItem("v1 Daily Plan")))
                .andExpect(jsonPath("$.tags[*].name").value(hasItem("v1 Task Checklist")))
