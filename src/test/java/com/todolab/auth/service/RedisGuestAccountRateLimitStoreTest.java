...
class RedisGuestAccountRateLimitStoreTest {
    ...
    void incrementAndGet_setsTtlOnFirstIncrement() {
        ...
        given(valueOperations.increment(startsWith("todolab:guest-rate-limit:client-key:")))
                .willReturn(1L);
        ...
        then(valueOperations).should().increment(startsWith("todolab:guest-rate-limit:client-key:"));
        then(redisTemplate).should().expire(startsWith("todolab:guest-rate-limit:client-key:"), any(Duration.class));
    }
}
